...
WITH rows AS (
    ...
    FROM (
        ...
        SELECT
            ...
            t.evt_index
        FROM
            uniswap_v3."UniswapV3Pool_evt_Swap" t
        INNER JOIN uniswap_v3."factory_polygon_evt_PoolCreated" f ON f.pool = t.contract_address
...
-- fill 2021 first pool created on 2021-12-20 17:54
SELECT dex.insert_uniswap_v3(
    '2021-12-20',
    '2022-01-01',
    (SELECT max(number) FROM ethereum.blocks WHERE time < '2021-12-20'),
    (SELECT max(number) FROM ethereum.blocks WHERE time <= '2022-01-01')
)
WHERE NOT EXISTS (
    SELECT *
    FROM dex.trades
    WHERE block_time > '2021-12-20'
